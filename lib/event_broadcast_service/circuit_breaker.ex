defmodule EventBroadcastService.CircuitBreaker do
  @moduledoc """
  ...
  ## Usage:
      ...
      CircuitBreaker.call(:kafka, fn ->
        Producer.publish("events.broadcast", event)
  ...
  @fuses %{
    redis: :event_broadcast_redis_fuse,
    mongodb: :event_broadcast_mongodb_fuse,
    kafka: :event_broadcast_kafka_fuse
  }
  ...
  defp fuse_options(service) do
    config = Application.get_env(:event_broadcast_service, :circuit_breaker, %{})
  ...
  defp emit_circuit_open_telemetry(service) do
    :telemetry.execute(
      [:event_broadcast_service, :circuit_breaker, :open],
      %{count: 1},
      %{service: service}
    )
  end
