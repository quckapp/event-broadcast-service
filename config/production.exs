...
# Run with: MIX_ENV=production mix run --no-halt
# =============================================================================
...
# Kafka - Production (for event distribution)
config :event_broadcast_service, :kafka,
  brokers: String.split(System.get_env("KAFKA_BROKERS") || "localhost:9092", ","),
  consumer_group: "event-broadcast-service-production",
  ssl: System.get_env("KAFKA_SSL") == "true",
  topics: %{
    events: System.get_env("KAFKA_EVENTS_TOPIC") || "quckapp-events-production",
    broadcasts: System.get_env("KAFKA_BROADCASTS_TOPIC") || "quckapp-broadcasts-production"
  }
